...
class MasterLabel(models.Model):
    """Reusable label bank for annotator dropdowns and AI-generated labels."""
    name = models.CharField(max_length=100, unique=True)
...
class Annotation(models.Model):
    ...
    def __str__(self):
        label = self.segmentation.label if self.segmentation else (self.label or "unlabeled")
        return f"Annotation {self.id} - {label} on {self.job_image}"
