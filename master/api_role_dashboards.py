...
class AnnotatorDashboardSerializer(serializers.Serializer):
    ...
    belum_di_anotasi = serializers.IntegerField()
...
class AnnotatorDashboardAPIView(APIView):
    ...
    def get(self, request):
        ...
        # Most assignment happens at JobProfile.worker_annotator. JobImage.annotator
        # is only filled later in some flows, so include both to keep mobile KPI sane.
        my_images = JobImage.objects.filter(
            Q(job__worker_annotator=user) | Q(annotator=user)
        ).distinct()
        ...
        stats = {
            ...
            'belum_di_anotasi': images_unannotated,
